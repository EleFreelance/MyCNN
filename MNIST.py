...
def get_training_data_set(num, onerow=False):
    ...
    return image_loader.load(
        onerow), label_loader.load()  # 第一个load是调用的load(self,onerow=False):，第二个是def load(self):，从参数可以看出
...
if __name__ == "__main__":
    ...
    train_data_set = train_data_set[:, np.newaxis, :, :]
    train_labels = np.array(train_labels)
    onepic = train_data_set[12]  # 取一个样本
    conv_test = CNN.conv_layer(1, 5, 3)
    conv_test.forward(onepic)
